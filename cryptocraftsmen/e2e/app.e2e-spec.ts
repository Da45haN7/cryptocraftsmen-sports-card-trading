...
describe('Starting tests for cryptocraftsmen', function() {
  ...
  it('website title should be cryptocraftsmen', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('cryptocraftsmen');
    })
  });

  it('network-name should be cryptocraftsmen-sports-card-trading@0.0.2-deploy.2',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('cryptocraftsmen-sports-card-trading@0.0.2-deploy.2.bna');
    });
  });

  it('navbar-brand should be cryptocraftsmen',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('cryptocraftsmen');
    });
  });
